  ...
  const payHandler = async () => {
    ...
    try {
      ...
      const transactionResponse = await createTransaction(
        {
          payment_id: form.payment === "4" ? 2 : form.payment, // PayOS dùng payment_id = 2 (BANKING) trong DB
          delivery_id: 1,
      ...
      const orderId = transactionResponse.data?.data?.id;
      
      // Nếu là PayOS (form.payment === "4"), tạo payment link và redirect trực tiếp đến PayOS
      if (form.payment === "4" && orderId) {
        try {
          const payOSResponse = await createPayOSPayment(orderId, userInfo.token, controller);
          const paymentUrl = payOSResponse.data?.data?.payment_url;
          if (paymentUrl) {
            dispatch(cartActions.resetCart());
            // Redirect trực tiếp đến PayOS
